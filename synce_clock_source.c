...
int synce_clock_source_add_source(struct synce_clock_source *clk_src,
		const char *clock_source_name, enum clk_type type)
{
	...
	clk_src->type = type;
	if (type == PORT) {
		clk_src->pointer.port = synce_port_create(clock_source_name);
		if (!clk_src->pointer.port)
			return -ENODEV;
	} else {
		clk_src->pointer.ext_src = synce_ext_src_create(clock_source_name);
		if (!clk_src->pointer.ext_src)
			return -ENODEV;
	}
			    ...
			    int is_extended, int recovery_time)
{
	...
	if (clock_source->type == PORT)
		return synce_port_init(clock_source->pointer.port, cfg,
				       network_option, is_extended,
				       recovery_time);
	return synce_ext_src_init(clock_source->pointer.ext_src, cfg,
				  network_option, is_extended);
}
...
void synce_clock_source_destroy(struct synce_clock_source *clock_source)
{
	...
	if (clock_source->type == PORT) {
		synce_port_destroy(clock_source->pointer.port);
		free(clock_source->pointer.port);
	} else {
		synce_ext_src_destroy(clock_source->pointer.ext_src);
		free(clock_source->pointer.ext_src);
	}
}
...
const char *synce_clock_source_get_name(struct synce_clock_source *clock_source)
{
	...
	if (clock_source->type == PORT)
		return synce_port_get_name(clock_source->pointer.port);
	return synce_ext_src_get_name(clock_source->pointer.ext_src);
}
...
*is_valid_clock_source(struct synce_clock_source *clock_source)
{
	if (clock_source && clock_source->type == PORT) {
		if (!clock_source->pointer.port->pc)
			return NULL;
		if (!is_valid_source(clock_source->pointer.port->pc))
			return NULL;
	}
	return clock_source;
}
				 ...
				 uint16_t *priority_count)
{
	...
	if (clock_source->type == PORT) {
		*priority_count = get_priority_params(
				clock_source->pointer.port->pc, priority_list);
		return get_ql_priority(clock_source->pointer.port->pc);
	} else {
		*priority_count = get_ext_src_priority_params(
				clock_source->pointer.ext_src, priority_list);
		return get_ext_src_ql_priority(clock_source->pointer.ext_src);
	}
}
