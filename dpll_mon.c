...
enum dpll_mon_state {
	...
	DPLL_MON_STATE_DEV_ID_GET_WAIT,
...
static void *dpll_mon_thread(void *data)
{
	...
	while (*state >= DPLL_MON_STATE_INIT_READY &&
		...
		nl_recvmsgs_default(dm->dev_sk);
		nl_recvmsgs_default(dm->rt_sk);
		       ...
		       int exist, int notify)
{
	...
	if (pin_state_valid  && pin->state != pin_state) {
		...
		if (pin->valid && pin->state != DPLL_PIN_STATE_SELECTABLE)
...
static int dpll_rt_recv(struct nl_msg *msg, void *arg)
{
	...
	nlh = nlmsg_hdr(msg);
	if (!nlh) {
		pr_err("failed parse nl header");
		return -EINVAL;
	}
	ret = nlmsg_parse(nlmsg_data(nlh), sizeof(*info), tb, IFLA_MAX, NULL);
	if (ret) {
	...
	if (tb[IFLA_DPLL_PIN]) {
		nla_for_each_nested(an, tb[IFLA_DPLL_PIN], rem) {
			if (nla_type(an) == DPLL_A_PIN_ID) {
				if (pin) {
					pin->id = nla_get_u32(an);
				} else {
					pin = find_pin(dm, nla_get_u32(an));
					if (pin)
...
static int dpll_mon_recv(struct nl_msg *msg, void *arg)
{
	...
	case DPLL_CMD_DEVICE_ID_GET:
		if (dm->state == DPLL_MON_STATE_DEV_ID_GET_WAIT) {
			dpll_mon_device_id_update(dm, tb);
...
static void init_pins(struct dpll_mon *dm)
{
	struct dpll_mon_pin *pin;
	int ret;

	ret = nl_rt_dump_links(dm->rt_sk, dpll_rt_recv, dm);
	if (ret >= 0)
	...
	STAILQ_FOREACH(pin, &dm->pins, list) {
		if (!pin->ifname && pin->valid == PIN_VALID) {
			ret = nl_dpll_pin_id_get(dm->dev_sk, dpll_mon_recv, dm,
	...
	ret = nl_dpll_pin_dump(dm->dev_sk, dpll_mon_recv, dm,
			       dm->family);
	if (ret >= 0)
...
static void state_step(struct dpll_mon *dm)
{
	...
	case DPLL_MON_STATE_INIT_READY:
		dpll_mon_state_set(dm, DPLL_MON_STATE_DEV_ID_GET_WAIT);
		ret = nl_dpll_device_id_get(dm->dev_sk, dpll_mon_recv, dm,
					    dm->family, dm->clock_id,
					    dm->module_name);
		if (ret >= 0)
	...
	case DPLL_MON_STATE_DEV_ID_READY:
		...
		ret = nl_dpll_device_get(dm->dev_sk, dpll_mon_recv, dm,
	...
	case DPLL_MON_STATE_PIN_DUMP_WAIT:
		...
		else if (dm->init_err % PIN_READY_TRIES == 0)
			dpll_mon_state_set(dm, DPLL_MON_STATE_DPLL_REINIT);
		break;
