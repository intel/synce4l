...
void set_clock_source_dnu_prio(struct synce_dev *dev,
			       struct synce_clock_source **skip, int skip_num)
{
	struct synce_clock_source *tmp;
	int i, skip_src;
	...
	LIST_FOREACH(tmp, &dev->clock_sources, list) {
		...
		if (skip_src)
			continue;
		synce_clock_source_prio_set(dev->dpll_mon, tmp, dev->dnu_prio);
	}
}
